...
- Prediction: **the model runs in a separate Python process.** WMA84 takes the 84 contiguous calendar days ending at `as_of_date` and returns a linearly weighted average of daily sales with the most recent day weighted highest; shorter history degrades through the cold-start ladder `FULL_MEAN_FALLBACK` / `MA_28_FALLBACK` / `MA_56_FALLBACK`, and all-zero data gives `ALL_ZERO`. Safety stock = predicted sales for the next two weeks = the daily rate × 14; purchase suggestion = max(0, safety stock − available), rounded up to the orderMultiple when one is set. A day's sales can be negative (return days pull the rate down).
  - The algorithm lives in `server/python/predict_core.py` (self-contained, only numpy/pandas) — **replacing the model means replacing that file**; the stdin/stdout contract is `server/python/main.py`; `server/src/modules/predictor.ts` is only the subprocess transport and holds no model logic.
  - The interpreter comes from `PREDICTOR_PYTHON` (it must be able to import numpy/pandas — a bare `python` may be a conda base env with a broken pandas); `main.py` self-checks with `<python> server/python/predict_core.py`.
...
- `python/` sits next to `src/`: the Python implementation of the prediction model, outside the tsc/vitest build. `predict_core.py` is the vendored algorithm and `main.py` is the I/O adapter.
